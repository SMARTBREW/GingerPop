...
export function wordCountLabel(value: string, limit: number): string {
  const words = countWords(value);
  const over = words > limit;
  return `${words}/${limit} words${over ? " — too long" : ""}`;
}

export function isOverWordLimit(value: string, limit: number): boolean {
